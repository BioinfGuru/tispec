...
```{r, echo = FALSE}
include_graphics(paste(wd,'density.png',sep = ''))
```
...
```{r, echo = FALSE}
include_graphics(paste(wd,'dist.png',sep = ''))
```
...
```{r, echo = FALSE}
include_graphics(paste(wd,'optimum.png',sep = ''))
```

The plot shows the top 5 optimal genes for tissue A. Arl11 and Tial1 are absolutely specific with high expression, while the other 3 genes are still highly specific for tissue A but their expression in other tissues is minimal. The plot shows all other tissues in which those genes are expressed so as to facilitate prediction of potential off target affect of any biological experiments (e.g. CRISPR). To confirm these findings simply extract the expression values of these genes from the input data. Notice the high expression in tissueA, and minimal expression elsewhere.
...
```{r, eval=FALSE}
corrPlots <- plotCorr(tissueA, c('Col4a3', 'Ifnb1'))
corrPlots$tauPlot
corrPlots$fracPlot
```

```{r, echo=FALSE}
include_graphics(paste(wd,'tauPlot.png',sep = ''))
```

By plotting the tau value against expression we see that as the specificity of a gene **for ANY tissue** increases, the expression of the gene in tissueA decreases. There is a negative correlation (r = -0.59) as reflected in the trendline. However, many tissueA absolutely specific genes (orange) still have relatively high expression in tissueA. We also see how genes of interest passed by the user are highlighted (pink): both are highly expressed and highly specific, although from this plot it is not known for which tissue they are specific because only the tau value was used.

```{r, echo=FALSE}
include_graphics(paste(wd,'fracPlot.png',sep = ''))
```

By plotting the tau expression value against expression we see that as specificity of a gene **for tissueA** increases above 0.1, there is actually little effect on expression. There is a positive correlation (r = 0.55). However, the trendline shows an initial strong positive correlation that plateaus above 0.1 specificity (frac). This is clearly shown by the wide range of expression of genes that are absolutely specific (orange) or highly specific (yellow) for tissueA. From this plot we can confirm that both genes passed by the user are highly expressed and highly specific to tissueA because the tau expression fraction was used. To extract genes that are both highly specific and highly expressed use [getOptimum](#getOptimum). Try passing the results of getOptimum to this function: in what area of the scatter plot do you think those genes will be? 

### Get a set of control genes {#getControls}

This function is useful for differentiating between two tissues. For example, to determine if a small sample of suprachiasmtic nucleus brain tissue has been contaminated with cortex brain tissue during tissue dissection. Performing qPCR on the top ranked genes returned by the function will confirm the presence/absence of the contaminant.
...
```{r, echo = FALSE}
include_graphics(paste(wd,'gene.png',sep = ''))
```
